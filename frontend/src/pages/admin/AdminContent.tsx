...
interface Resource { _id: string; title: string; url: string; type: "youtube" | "website"; subject: string; semester: number; branch: string; createdAt: string; user?: { name: string; email: string }; }

export function AdminContent() {
  const [tab, setTab] = useState<"notes" | "pyqs" | "ct-pyqs" | "assignments" | "resources">("notes");
  ...
  const [uploadType, setUploadType] = useState<"note"|"pyq"|"ct-pyq"|"assignment"|"resource">("note");
  const [uploadMethod, setUploadMethod] = useState<"link"|"file">("link");
  const [resourceType, setResourceType] = useState<"youtube"|"website">("youtube");
  ...
  const loadContent = async () => {
    try {
      const [n, p, c, a, r, s] = await Promise.all([
        api.get("/admin/notes"),
        api.get("/admin/pyqs"),
      ...
      if (uploadMethod === "link") {
        if (uploadType === "note") {
          ...
          await api.post("/admin/resources/link", {
            title: finalTitle,
            url: formData.driveUrl, // Using driveUrl field for the URL
            type: resourceType,
            subject: formData.subject,
            semester: formData.semester,
            syllabus: formData.syllabus,
  ...
  const deleteItem = async (id: string, type: "notes"|"pyqs"|"ct-pyqs"|"assignments"|"resources", title: string) => {
  ...
  const handleDownload = async (item: any, type: "notes"|"pyqs"|"ct-pyqs"|"assignments"|"resources") => {
    if (item.driveUrl || item.url) {
      window.open(item.driveUrl || item.url, "_blank");
      return;
    }
  ...
  const handleView = async (item: any, type: "notes"|"pyqs"|"ct-pyqs"|"assignments"|"resources") => {
    if (item.driveUrl || item.url) {
      window.open(item.driveUrl || item.url, "_blank");
      return;
    }
  ...
  const renderTable = (items: any[], type: "notes"|"pyqs"|"ct-pyqs"|"assignments"|"resources") => (
    ...
    <div className="space-y-6 max-w-6xl mx-auto p-4 sm:p-8">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-zinc-400">Manage notes, previous year questions, assignments, and study resources.</p>
        </div>
        
        <button
          onClick={() => { setUploadType(tab === "notes" ? "note" : tab === "pyqs" ? "pyq" : tab === "ct-pyqs" ? "ct-pyq" : tab === "assignments" ? "assignment" : "resource"); setUploadMethod("link"); setShowUpload(true); }}
      ...
      <div className="flex items-center gap-1 sm:gap-2 border-b border-[#222] overflow-x-auto scrollbar-hide">
        {(["notes", "pyqs", "ct-pyqs", "assignments", "resources"] as const).map((t) => (
          <button
            key={t}
            onClick={() => setTab(t)}
          ...
          >
            {t === "notes" ? "Notes" : t === "pyqs" ? "PYQs" : t === "ct-pyqs" ? "CT PYQs" : t === "assignments" ? "Assignment Solutions" : "Resources"}
          </button>
        ))}
      </div>
      ...
      <div className="rounded-2xl border border-[#222] bg-[#0A0A0A] shadow-xl overflow-hidden">
        ...
        {tab === "resources" && renderTable(resources, "resources")}
      </div>
            ...
            <form onSubmit={handleUploadSubmit} className="p-6 space-y-5">
              ...
              <div>
                ...
                <div className="flex bg-[#111] rounded-lg p-1 gap-1 border border-[#222] overflow-x-auto scrollbar-hide">
                  {(["note", "pyq", "ct-pyq", "assignment", "resource"] as const).map((type) => (
                    <button
                      key={type}
                      type="button"
                      onClick={() => { setUploadType(type); if(type === "resource") setUploadMethod("link"); }}
                    ...
                    >
                      {type === "note" ? "Note" : type === "pyq" ? "PYQ" : type === "ct-pyq" ? "CT PYQ" : type === "assignment" ? "Solution" : "Resource"}
                    </button>
                  ))}
                </div>
              </div>
              ...
              <div className="flex gap-6 mb-2">
                <label className="flex items-center gap-2 text-sm font-bold text-zinc-300 cursor-pointer hover:text-white transition-colors">
                  ...
                  {uploadType === "resource" ? "Resource URL" : "Drive Link"}
                </label>
                {uploadType !== "resource" && (
                  <label className="flex items-center gap-2 text-sm font-bold text-zinc-300 cursor-pointer hover:text-white transition-colors">
              ...
              {uploadType === "resource" && (
                <div className="flex gap-6 mb-4 p-3 bg-[#111] rounded-lg border border-[#222]">
                  <label className="flex items-center gap-2 text-sm font-bold text-zinc-300 cursor-pointer hover:text-white transition-colors">
                    <input type="radio" checked={resourceType === "youtube"} onChange={() => setResourceType("youtube")} className="accent-[#FF9000] w-4 h-4" />
                    YouTube Channel
                  </label>
                  <label className="flex items-center gap-2 text-sm font-bold text-zinc-300 cursor-pointer hover:text-white transition-colors">
                    <input type="radio" checked={resourceType === "website"} onChange={() => setResourceType("website")} className="accent-[#FF9000] w-4 h-4" />
                    Website / Article
                  </label>
                </div>
              )}
              ...
              <div>
                ...
                <input required type="text" value={formData.title} onChange={e => setFormData({...formData, title: e.target.value})} className="w-full bg-[#111] border border-[#333] rounded-lg px-4 py-2.5 text-white focus:outline-none focus:border-[#FF9000] transition-colors" placeholder={uploadType === "resource" ? "e.g. Neso Academy" : "e.g. Operating Systems Unit 1"} />
              </div>

              {uploadMethod === "link" ? (
                <div>
                  <label className="block text-xs font-bold text-zinc-400 mb-1.5 uppercase tracking-wider">{uploadType === "resource" ? "Resource URL" : "Google Drive Link"}</label>
