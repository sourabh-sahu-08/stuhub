...
export function CtPyqPage() {
  ...
  const fetchSubjectOptions = async (branch: string, sem: number, syllabus: string) => {
    try {
      const response = await api.get(`/pyq/subjects/${branch}/${sem}?syllabus=${syllabus}`);
  ...
  // View file inline
  const handleView = async (paper: PyqPaper) => {
    try {
      if (paper.driveUrl) {
        window.open(paper.driveUrl, "_blank");
        return;
      }
      const response = await api.get(`/pyq/download/${paper._id}`, {
        responseType: "blob"
      });
      const blob = new Blob([response.data], { type: paper.mimeType });
  ...
  // Download file
  const handleDownload = async (id: string, fileName: string, mimeType: string | undefined, driveUrl?: string) => {
    if (driveUrl) {
          ...
          >
            <div>
              ...
              <p className="mt-2 text-sm text-zinc-400">
                Select your engineering branch to browse past exam question papers.
              </p>
            </div>
                ...
                {papers.filter(p => !subjectOptions.some(s => s.name.toLowerCase() === p.subject?.toLowerCase() || s.code.toLowerCase() === p.subject?.toLowerCase())).length > 0 && (
                  <div className="flex flex-col space-y-3">
                    ...
                    <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                      {papers.filter(p => !subjectOptions.some(s => s.name.toLowerCase() === p.subject?.toLowerCase() || s.code.toLowerCase() === p.subject?.toLowerCase())).map(paper => (
                        <div
                          key={paper._id}
